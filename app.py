...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS session_rosters (roster_id TEXT PRIMARY KEY, session_id TEXT NOT NULL, entity_type TEXT, player_id_p1 TEXT, player_id_p2 TEXT, display_label TEXT, group_id TEXT DEFAULT 'A', seed_index INTEGER DEFAULT 0, initial_mmr REAL DEFAULT 3.0, initial_rd REAL DEFAULT 350.0, is_checked_in INTEGER DEFAULT 0, is_defected INTEGER DEFAULT 0, matches_played INTEGER DEFAULT 0, matches_won INTEGER DEFAULT 0, matches_lost INTEGER DEFAULT 0, matches_tied INTEGER DEFAULT 0, standing_points INTEGER DEFAULT 0, games_for INTEGER DEFAULT 0, games_against INTEGER DEFAULT 0, net_game_diff INTEGER DEFAULT 0, points_for INTEGER DEFAULT 0, points_against INTEGER DEFAULT 0, net_point_diff INTEGER DEFAULT 0, consecutive_sit INTEGER DEFAULT 0, is_qualified INTEGER DEFAULT 0, knockout_seed INTEGER, FOREIGN KEY (session_id) REFERENCES sessions(session_id) ON DELETE CASCADE)''')

    # Safe col checks omitted for speed; table creation guarantees structure in clean builds.
...
nav = st.sidebar.radio("Navigation Console", ["📊 The Dashboard", "🎾 Log Matches", "🧠 Session Logic (V16.2 PROD)", "🏆 Tournament Desk (Delayed)", "📜 Historical Matches", "👥 Player Roster & Calibration", "🏢 Venues & Regions", "🌐 Hawking Engine", "⚙️ Global Config", "📄 RYFT Documentation"])
    ...
    if mode == "➕ Create Session":
        ...
        def render_session_workspace(s_data):
            ...
            if sub_nav == "📋 Check-In Gate":
                ...
                if completed:
                    # SEQUENTIAL IN-MEMORY STATE FOR UI CARDS
                            ...
                            with st.form(f"edit_comp_{m['session_match_id']}"):
                                if fmt_category == "RACE_GAMES":
                                    ...
                                    if st.form_submit_button("Update Points"):
                                        if (new_a + new_b) != fmt_info["total_points"]: st.error(f"Score must total {fmt_info['total_points']}.")
                                        else:
