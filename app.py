...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS locations (
        ...
        active_venues_count INTEGER DEFAULT 0, median_latent_mmr REAL DEFAULT 3.000,
        highest_player_mmr REAL DEFAULT 3.000, lowest_player_mmr REAL DEFAULT 3.000, is_normalized INTEGER DEFAULT 0,
        updated_at TEXT, is_active INTEGER DEFAULT 1, FOREIGN KEY (parent_id) REFERENCES locations(location_id) ON DELETE SET NULL
    )''')
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS matches (
        ...
        applied_g_buffer REAL DEFAULT 1.000, delta_r_p1 REAL NOT NULL, delta_r_p2 REAL DEFAULT 0.0, delta_r_p3 REAL NOT NULL,
        delta_r_p4 REAL DEFAULT 0.0, guardrails_summary TEXT DEFAULT '[]', match_timestamp TEXT NOT NULL,
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS match_logs (
        ...
        FOREIGN KEY (match_id) REFERENCES matches(match_id) ON DELETE CASCADE, FOREIGN KEY (player_id) REFERENCES players(player_id) ON DELETE CASCADE
    )''')
    ...
    # Master Configuration Matrix with New Provisional Overrides
    master_params = [
        ...
        ("MARGIN_BASE", 0.80, 1, "Margin Floor Factor", "Min score factor for close matches.", "Points floor for tight 7-6 finishes.", "Margins & Formats"),
        ...
        ("PROVISIONAL_ABSORPTION_ALPHA", 0.45, 1, "Rightsizing Velocity", "Speed toward performance rating.", "Higher = faster unranked rightsizing.", "Margins & Formats"),
        ("MAX_24H_EXCHANGE_CAP", 0.150, 1, "24H Casual Cap", "Net transfer ceiling between 4 players.", "Prevents farming.", "Anti-Farming"),
        ("PROVISIONAL_CAP_MULTIPLIER", 2.5, 1, "Provisional Cap Relaxer", "Multiplier on 24H cap for PRs.", "Allows 0.375 point movement for PRs.", "Anti-Farming"),
        ...
        ("PROV_RD_SHRINK_MULTIPLIER", 0.35, 1, "Provisional RD Shrink Modifier", "Slows RD drop for unrated players.", "Lower values (e.g. 0.30) keep players provisional longer.", "Accuracy & Calibration"),
        ("PROV_ACCURACY_GAIN_MULTIPLIER", 0.40, 1, "Provisional Accuracy Gain Cap", "Restricts accuracy gain during placement.", "Lower values (e.g. 0.30) cap visual accuracy progression.", "Accuracy & Calibration"),
...
class RyftV16:
    ...
    def calc_accuracy(rd, m_count, opp_count, is_prov, k_bridges, cfg):
        ...
        # FIX 2: Decoupled Accuracy Gain for Provisionals
        if is_prov:
    ...
    def compute_match(cls, p1, p2, p3, p4, s_a, s_b, g_w_raw, g_l_raw, fmt_id, v_id, is_singles, is_dry=False):
        ...
        for p, is_a, partner, opp_rd in participants:
            ...
            # FIX 3: Provisional 24H Exchange Cap Parameterization
            ...
            # FIX 2: Decoupled RD shrinkage for Provisionals
            ...
            # FIX 1: Honor manual override for tier/provisional status
            ...
            # Persist manual verification
            if is_manual_override and tier == "PROVISIONAL":
                tier = "VERIFIED"
...
st.sidebar.title("⚡ RYFT Engine V.16")
...
# ------------------------------------------------------------------------------
# TAB 2: LOG MATCHES (WITH DATE & TIME - FIX 4)
# ------------------------------------------------------------------------------
    ...
    if do_dry or do_save:
        ...
        if do_save:
            ...
            # FIX 4: Precise user-defined Match Timestamp concatenation
            ...
            for pr in sim_out["res"]:
                ...
                ''', (
                    ...
                    pr["acc"], pr["tier"], pr["prov"], ts, pr["pid"]
                ))
...
# ------------------------------------------------------------------------------
    ...
    # FIX 4: Native Timestamp Querying based on User's explicit Date & Time inputs
        ...
        for m in matches_list:
            with st.container():
                ...
                <div style="border: 1px solid #cbd5e1; border-radius: 8px; padding: 12px; margin-bottom: 10px; background-color: #f8fafc;">
                    ...
                    <strong>Venue:</strong> {m['venue_name']} ({m['city_name']}) | <strong>Format:</strong> {m['format_name']}
                </div>
                """, unsafe_allow_html=True)
                col_res1, col_res2 = st.columns(2)
                with col_res1:
                    st.write(f"**🔵 Team A:** {m['p1_name']} & {m['p2_name'] if not m['is_singles'] else ''}")
                    st.write(f"Δ: `{m['delta_r_p1']:+0.4f}` | `{m['delta_r_p2']:+0.4f}`")
                with col_res2:
                    st.write(f"**🔴 Team B:** {m['p3_name']} & {m['p4_name'] if not m['is_singles'] else ''}")
                    st.write(f"Δ: `{m['delta_r_p3']:+0.4f}` | `{m['delta_r_p4']:+0.4f}`")
                st.write(f"**Scorelines:** Sets: `{m['set_scores_json']}`")
                
                with st.expander("🔍 Inspect 25-Bit Algorithmic Drill Down"):
...
# ------------------------------------------------------------------------------
# TAB 4: PLAYER ROSTER & CALIBRATION (WITH PERSISTENT MANUAL VERIFICATION OVERRIDE)
# ------------------------------------------------------------------------------
elif nav == "👥 Player Roster & Calibration":
    st.title("Players Directory & Calibration Roster")

    conn = get_db_connection()
    df_display = pd.read_sql_query("SELECT * FROM players WHERE calibration_tier != 'INACTIVE' ORDER BY latent_mmr DESC", conn)
    conn.close()
            ...
            with st.form("add_player_form"):
                ...
                if st.form_submit_button("Create Player Profile"):
                    ...
                    conn.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, 350.0, ?, ?)
                    """, (p_uuid, p_name, base_r, c_row["location_id"], c_row["country_code"], base_r, base_r, cat_sel, datetime.now(timezone.utc).isoformat()))
                    conn.commit()
                    conn.close()
        ...
        with st.expander("✏️ Inspect & Edit Player", expanded=False):
            ...
            if all_p:
                ...
                with st.form("edit_player_form"):
                    ...
                    st.markdown("##### 🛡️ Calibration Overrides")
                    ...
                    if st.form_submit_button("Save Overrides"):
                        conn.execute("""
                            UPDATE players SET 
                                ...
                                is_manually_verified = ?, is_anchor = ?
                            WHERE player_id = ?
                        """, (e_name, e_mmr, e_mmr, 1 if e_man_ver else 0, 1 if e_anc else 0, p_pick))
                        
                        # If manually verified, strictly enforce Verified/Anchor status on save
...
# ------------------------------------------------------------------------------
    ...
    st.info("Full CRUD module active. Reconcile venues, delete nodes, view density metrics.")

# ------------------------------------------------------------------------------
# TAB 6: HAWKING REGIONAL CONTROL
# ------------------------------------------------------------------------------
elif nav == "🌐 Hawking Regional Control":
    st.title("Hawking Macro Normalization")
    st.info("Zero-mix island calibrations and macro offset deployments enabled.")

# ------------------------------------------------------------------------------
# TAB 7: GLOBAL CONFIG (WITH FIX 2 & 3: PROVISIONAL CAPS & CONTRACTION OFFSETS)
# ------------------------------------------------------------------------------
